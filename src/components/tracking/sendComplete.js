...
import ImgBoxComplete from "./../../assets/boxComplete.png";
import ImgBoxComplete2 from "./../../assets/boxComplete2.png";
import ImgQrcode from './../../assets/blackberry-qr-code-variant.png'
import { AsyncStorage } from 'react-native';
import { domain } from '../../config/configApp'

export default class sendComplete extends React.Component {
  ...
  componentDidMount = async () => {
    await AsyncStorage.getItem('phone', (err, result) => {
      ...
      console.log('####pageTrackingComplete', this.state.phones);
    });
  ...
  callApiMyParcelList(phones) {
    axios({
      method: 'get',
      url: `${domain}/tracking/myparcellist/${phones}/Completed`,
      ...
      <View style={styles.popupContent}>
        <ScrollView contentContainerStyle={styles.modalInfo}>
          <View style={{ flex: 1, flexDirection: 'row', padding: wp('5%'), alignItems: 'center' }}>
            <View style={styles.cardImage}>
              <Image style={styles.imageBox} source={ImgBoxComplete} />
              <Text style={{ fontSie: hp('2%'), textAlign: 'center', color: '#999999', marginTop: hp('1.2%') }}>{this.state.userSelected.total_box} กล่อง</Text>
            </View>
    ...
    if (this.state.foundParcel) {
      ...
      MyParcelList =
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
          <Image style={{ width: wp('30%'), height: hp('15%'), resizeMode: 'contain', marginBottom: hp('4%') }} source={ImgBoxComplete2} />
